...
export async function placeOrder(symbol: string, side: 'BUY' | 'SELL', quantity: number): Promise<BinanceOrder> {
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            throw new Error(`${error.response?.data.code}: ${error.response?.data.msg}`);
        } else {
            throw error;
        }
    }
